 ...
 */
public class ContextMeasureFilter extends LeoBaseAnnotator {
  ...
  public void annotate(JCas aJCas) throws AnalysisEngineProcessException {
    ...
    Collection<TermContext> contexts = AnnotationLibrarian.getAllAnnotationsOfType(aJCas, TermContext.type, false);
    for (TermContext context : contexts) {    	
      try {
        // Window window = (Window) windowService.buildNonWhitespaceTokenWindow(context);
        Annotation window = context.getWindow();
        
        // System.out.println("Value: " + context.getCoveredText());
        // System.out.println("Window: " + window.getCoveredText());
               
        if (hasAnchorPatternMatch(context) ||
            hasWindowPatternMatch(window) ||
            hasPrePatternMatch(context, window) ||
            hasPostPatternMatch(context, window)) {
        	context.removeFromIndexes();
        }
      } catch (Exception e) {
  ...
  public ContextMeasureFilter setRegexFilePath(String regexFilePath) {
